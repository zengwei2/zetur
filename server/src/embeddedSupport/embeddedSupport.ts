...
// 根据 position 找出当前的 languageId 的值
function getLanguageAtPosition(document: TextDocument, regions: EmbeddedRegion[], position: Position): LanguageId {
  // offset: 81
  // regions: [
  //   { languageId: 'vue-html', start: 10, end: 43, type: 'template', attrs: {} },
  //   { languageId: 'javascript', start: 64, end: 84, type: 'script', attrs: {} },
  //   { languageId: 'css', start: 102, end: 103, type: 'style', attrs: {} },
  // ]
  const offset = document.offsetAt(position);
  for (const region of regions) {
    // 判断当前 offset 是在哪个 start 和 end 范围，然后找出 languageId 是什么
