...
export async function createProjectService(
  ...
  // languageModes: { // 还有一些属性
  //    modes: { vue: { xxx }, css: { xxx }, javascript: { xxx } },
  //    serviceHost: {  },
  //    modelCaches: [{ xxx }],
  //    documentRegins: { xxx }
  // }
  ...
  return {
    ...
    // position: 当前光标位置({ line: 6, character: 3 })
    async onCompletion({ textDocument, position, context }) {
      // 获取到当前文件内容( 内容字符串 )
      // uri: "file:///c%3A/Users/zengwei/Desktop/vue/Vue3/Vue3-my/index.vue" 扩展开发宿主文件
      // doc: { _languageId: 'vue', _content: '内容字符串', _lineOffsets: [xxx], version: 1, xxx }
      const doc = documentService.getDocument(textDocument.uri)!;
      // 关键代码：这里做分割文件，识别到不同区块的 mode 对象( 识别 script、template、style 的 mode )
      //       这里的 mode 就是 modules/script|template||style
      // 为什么分割 ?：1 个文件夹下存在多个语言的情况
      // mode: { doComplete: f()、doHover: f(),xxx }
      ...
      if (mode && mode.doComplete) {
        ...
        // doComplete方法会在对于 modes 下 xxx 文件夹下的 index.ts
